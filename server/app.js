...
io.sockets.on('connection', function(socket) {
    ...
    socket.on('message', function(message) {
        ...
        if (data[0] === 'mac') {
            ...
            socket.emit('message', 'Connection done');
            _is_MAC = false;
            setTimeout(function() {
                _is_MAC = true
            }, 500);
        ...
        if (data[0] === 'hexa') {
            ...
            console.log('set hexa to ', val.toString(16));
            helicoJS.setHexa(val);
            return;
        }
        ...
        var vyFront = parseInt(data[4]);
        var vyBack = parseInt(data[5]);
        var vBalance=parseInt(data[6]);

        //console.log(vx, vz);

        helicoJS.setBTDATA(vx, vz, vy, vTrim, vyFront, vyBack, vBalance);
        // console.log('#');
        return;
